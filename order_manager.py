...
class OrderManager:
    def __init__(self, order_queue: asyncio.Queue, broadcast_cb=None, on_order_completed=None):
    ...
    async def start(self, shutdown_event: asyncio.Event):
        ...
        while not shutdown_event.is_set():
            try:
                ...
                logger.error(f"Order Manager queue error: {e}")
                continue
            ...
            try:
                ...
                # Calculate initial estimated slippage price
                if signal.side == 'buy':
                ...
                if mode == 'live':
                    ...
                    # PRE-TRADE SYNC
                    try:
                        live_balances = await asyncio.wait_for(client.get_balances(), timeout=10.0)
                        ...
                        logger.warning(f"Pre-trade balance sync failed: {sync_err}")

                    # Execute market order using USD execution price mapping with a 30s timeout guard
                    try:
                        ...
                        logger.error(f"Live order placement timed out after 30s on CoinDCX for {signal.symbol} (phantom fill possible — position reconciliation loop will monitor exchange balance).")
                        resp = None
                    except Exception as ex_err:
                        logger.error(f"Live order placement error on CoinDCX for {signal.symbol}: {ex_err}")
                        resp = None
                ...
                # Recalculate PnL using finalized execution_price and fee
                if success and signal.side == 'sell':
                ...
                if success:
                    ...
                    logger.error("Trade failed (insufficient funds/asset or exchange error).")
                ...
                if self.on_order_completed:
                    try:
                        res = self.on_order_completed(
                            signal.symbol, signal.side, signal.amount, execution_price, success, getattr(signal, 'label', '')
                        )
                        if asyncio.iscoroutine(res):
                            await res
                    except Exception as cb_err:
                        logger.error(f"Error in on_order_completed callback: {cb_err}")
